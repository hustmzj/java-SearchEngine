...
public class DocumentBuilder extends AbstractDocumentBuilder {
    ...
    public AbstractDocument build(int docId, String docPath, AbstractTermTupleStream termTupleStream) {
        ...
        while (tup != null) {
            doc.addTuple(tup);
            tup = termTupleStream.next();    //调用层层过滤机制
        }
    ...
    public AbstractDocument build(int docId, String docPath, File file) {
        ...
        try {
            BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file)));    //BufferedReader将文件由字节流转化为字符流
            stream = new StopWordTermTupleFilter(new LengthTermTupleFilter(new PatternTermTupleFilter(new TermTupleScanner(reader))));   //装饰者模式层层过滤
